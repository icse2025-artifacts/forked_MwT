...
def get_args():
    ...
    parser.add_argument('--model', type=str, choices=['vgg16', 'resnet18', 'mobilenet'], required=True)
...
def _test_with_modular_metrics(model, test_dataset):
    ...
    return n_correct / total_labels, mean_list(loss_log), mean_list(cohesion_log), mean_list(coupling_log), mean_list(
        krr_log)
...
def main():
    ...
    if model_name == 'vgg16':
        model = vgg16(pretrained=False, num_classes=num_classes).to('cuda')
    elif model_name == 'resnet18':
        model = ResNet18(num_classes=num_classes).to('cuda')
    elif model_name == 'mobilenet':
        model = MobileNet(num_classes=num_classes).to('cuda')
    else:
        raise ValueError
