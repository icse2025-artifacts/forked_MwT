...
def make_layers(cfg: List[Union[str, int]], batch_norm: bool = False, module_layer_masks=None,
                keep_generator=True) -> nn.Sequential:
    ...
    for v in cfg:
        if v == 'M':
            ...
            if in_channels == 3 and len(layers) == 0:
                ...
                pre_layer_mask = module_layer_masks[
                    mask_layer_idx - 1] if mask_layer_idx > 0 else first_norm_conv_pseudo_mask
                ...
                mask_conv_bn = MaskConvBN(in_channels=in_channels, out_channels=v, kernel_size=3, padding=1,
                                          module_layer_masks=[pre_layer_mask, cur_layer_mask],
...
def get_module_param(module, module_layer_masks, model_param, keep_generator=True):
    ...
    for each_layer_idx, each_layer in module.features._modules.items():
        ...
        for layer_param_name in ['conv.weight', 'conv.bias', 'bn.weight', 'bn.bias', 'bn.running_mean',
                                 'bn.running_var']:
...
def _vgg(arch: str, cfg: str, batch_norm: bool,
    ...
    module = VGG(make_layers(module_cfg, batch_norm=batch_norm, module_layer_masks=module_layer_masks,
                             keep_generator=keep_generator),
