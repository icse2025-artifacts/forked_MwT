        ...
        for data_loader, phase in zip([train_loader, test_loader], ['train', 'test']):
            if not is_cal_modular_metrics:
                ...
                if phase == 'train':
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        log_acc, log_loss, log_loss_ce, log_loss_cohesion, log_loss_coupling, log_loss_l1, log_kernel_rate, log_cohesion, log_coupling = _test_with_modular_metrics(
                            model, data_loader)
...
def loss_func_cos(masks, labels):
    ...
    mask_sim_ground_truth = mask_sim_ground_truth[
        torch.triu(torch.ones_like(mask_sim_ground_truth, device=DEVICE), diagonal=1) == 1]
...
def main():
    ...
    if model_name == 'vgg16':
        ...
        model = SimCNN(num_classes=num_classes).to(DEVICE)
    elif model_name == 'rescnn':
        model = ResCNN(num_classes=num_classes).to(DEVICE)
    else:
        raise ValueError
